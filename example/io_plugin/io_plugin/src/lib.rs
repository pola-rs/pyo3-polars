...
impl RandomSource {
    ...
    fn new_source(columns: Vec<PySampler>, size_hint: Option<usize>, n_rows: Option<usize>) -> Self {
    ...
    fn schema(&self) -> PySchema {
        let schema = self.columns.iter().map(|s| {
    ...
    fn set_with_columns(&mut self, columns: Vec<String>) {
        ...
        let indexes = columns.iter().map(|name| {
            schema.index_of(name.as_ref()).expect("schema should be correct")
        }).collect();
    ...
    fn next(&mut self) -> PyResult<Option<PyDataFrame>> {
        if self.n_rows > 0 {
            ...
            let s_iter = if let Some(idx) = &self.with_columns {
                Box::new(idx.iter().copied().map(|i| &self.columns[i])) as Box<dyn Iterator<Item=_>>
            } else {
                Box::new(self.columns.iter())
            };

            let columns = s_iter.map(|s| {
            ...
            if let Some(predicate) = &self.predicate {
                df = df.lazy().filter(predicate.clone())._with_eager(true).collect().map_err(PyPolarsErr::from)?;
            }
...
fn io_plugin(m: &Bound<PyModule>) -> PyResult<()> {
    ...
    m.add_wrapped(wrap_pyfunction!(samplers::new_bernoulli)).unwrap();
    m.add_wrapped(wrap_pyfunction!(samplers::new_uniform)).unwrap();
